...
const features = [
  ...
  const Features = () => {
    return (
      <section className="bg-white min-w-full h-105 py-12 px-14 flex flex-wrap justify-around gap-6 mr-4 ">
        {features.map((feature, idx) => (
          <div key={features.id} className="bg-white shadow-lg rounded-md w-80 p-4">
            <img src={feature.image} alt={feature.title} className="w-full h-60 object-cover rounded-md" />
            <h3 className="text-2xl font-semibold mt-4">{feature.title}</h3>
            <p className="text-lg mt-2">{feature.description}</p>
            
            <Link to={feature.link}>
              <button type = "button" className="mt-6 px-4 py-2 bg-black text-white aljustify-center text-sm rounded">Read More</button>
            </Link>
          </div>
        ))}
      </section>
